...
/// Recommended default number of concurrent upload/download requests.
///
/// Uses [`std::thread::available_parallelism`], which respects cgroup CPU
/// quotas and CPU affinity, unlike a raw physical-core count. This matters for
/// the constrained / containerized environments `s3m` is designed to run in: a
/// container limited to 2 CPUs reports `2` here even on a 64-core host. Two
/// units are reserved for the async runtime and the main task, and the result
